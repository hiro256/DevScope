...
Last successful slow-scan diagnostics remain in memory for that worker session; fast scans and failed diagnostic attempts do not clear them. They remain non-persisted performance hints.

A read-only, on-demand assessment may classify the heaviest diagnosed root-level subtrees as Activity exclusion candidates. It uses Git ignore and tracked-file queries as source truth, retaining generated-output names only as supplemental hints. Tracked source-like paths are not candidates, force-added ignored paths and `.devscope` local state require review, and Git observation failures never produce a safe candidate. The assessment neither changes configuration nor runs on the normal scan path.

A heavy root-level directory that remains `ReviewRequired` may be diagnosed one level deeper on demand. Its direct children use the same advisory Git assessment, so ignored untracked runtime subdirectories can be distinguished from tracked Current Work or Config content. The drill-down is non-persisted and non-recursive; child diagnostics are not themselves drilled down, and tracked content is never silently excluded.

`[activity].exclude` is explicit project policy for suppressing Git worktree scan hints. It accepts only literal project-relative file or subtree paths, not Git ignore syntax, globs, negation, or a project-root exclusion. Built-in `.git` and `target` safeguards remain mandatory. It is separate from `[verify].exclude`: Activity exclusion does not filter Git Activity display or Build/Test freshness. Explicitly excluding tracked content is allowed but can hide relevant worktree-change hints; candidate assessment may inform future human or AI edits, but DevScope does not change Config automatically. The TUI keeps its Build/Test Config as a startup snapshot: verify command and freshness settings changed during a session take effect on the next TUI start. Only activity exclusions are live-reloaded for worktree observation, rebuilding the detector baseline when they change.

Safe candidate assessments may be converted on demand into read-only Activity exclusion proposals. A proposal records a project-relative path, its diagnostic source, and its Git safety reasons; it never mutates Config. Existing exclusions are omitted, and a reviewed root directory may contribute proposals from one diagnostic drill-down level only. `devscope activity suggest-excludes` collects this diagnostic in its own on-demand CLI process; it does not read TUI worker memory. A Skill may maintain `[activity].exclude` only after explicit human approval of an exact proposal path; Config editing and diagnostic reruns remain separate steps, and disappearance of a proposal is not benchmark proof.

An external fixture with 2,000 ignored generated files surfaced `generated` as a Git-safe root proposal. After explicit fixture-only approval added that exact path, rerunning the diagnostic produced no proposal; the observed 63 ms is an approximate diagnostic hint, not a benchmark.
